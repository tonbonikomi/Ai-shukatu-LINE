...
[LINE Developers コンソール](https://developers.line.biz/console/) にログインし、
左側に**プロバイダーの一覧**が出るか見てください。

**「プロバイダー は見つかりません」と出た場合**、そのLINEアカウントには既存プロバイダーの
権限がありません（作業する人が既存の送客管理を作った本人でないときに起こります）。
取れる道は2つです。

| | やること | 得るもの / 失うもの |
|---|---|---|
| **A（推奨）** | 既存プロバイダーの Admin に、**「権限設定」タブ →「メールで招待」→ Admin** で招待してもらう | 既存の送客管理と userId が揃い、稼働者との自動連結が効く |
| **B** | 自分のアカウントで**新規プロバイダーを作る** | すぐ着手できる。ただし既存とは userId が別になり、紹介料の突合が手作業になる |

**Aを選ぶなら、招待が届くまでここで止まります。** 待てない事情があるならBでも
コミュニティ単体は問題なく動きますが、**後から変更できない**ので決めてから進んでください。

> 判断の材料: 必須なのは「一柳」と「一柳ログイン」が**互いに**同じプロバイダーにあることだけです。
> 既存の送客管理と同じである必要が生じるのは、**紹介料を既存の即払い基盤に自動で乗せたい場合**です。
> 詳しくは [調査結果 §3](line-platform-research.md)。

> ⚠️ どちらの道でも、**プロバイダーの Admin は必ず2人以上**にしてください。
> 1人だけだと、その人が抜けたときにチャネルを触れなくなります。
> グループ管理用の業務アカウントと同じ理屈です。

### 1-1. 使うプロバイダーを開く

**プロバイダーが見えている場合**は、既存の送客管理LIFF `2010483281-…` がどれに属するかを特定します。

> **LIFF ID のハイフンより前の数字は、そのLIFFが乗っている LINEログインチャネルのチャネルIDです。**
> つまりチャネルIDが `2010483281` の LINEログインチャネルを探し、それを含むプロバイダーが正解です。

### 1-2. 一柳（Messaging API）を作る

対象のプロバイダーを開いて「新規チャネル作成」→ **Messaging API** → チャネル名は「一柳」

### 1-3. 一柳ログイン（LINEログイン）を作る

**同じプロバイダーで**もう1つ「新規チャネル作成」→ **LINEログイン** → チャネル名は「一柳ログイン」
アプリタイプは **ウェブアプリ** にチェック。

### 1-4. LIFFを追加する

「一柳ログイン」チャネル → **LIFF** タブ → 「追加」
...
| スコープ | **profile** と **openid** |
| ボットリンク機能 | **On (Aggressive)** → 一柳を選択 |

**ここが同一プロバイダーのその場チェックになります。**
ボットリンク機能のプルダウンに「一柳」が出てこなければ、2つのチャネルが別プロバイダーにあります。
この機能は同一プロバイダー内の Messaging API チャネルしか選べないためです。出てこなければ作り直してください。

### 1-5. LIFF ID をメモ

`2010xxxxxx-yyyyyyyy` の形で発行されます。これが入口リンクの中身になります。

> ⚠️ **このLIFFアプリは今後削除・再作成しないでください。** IDが変わると配布済みの入口リンクが全部死にます。
> 中身（エンドポイントURL・項目・デザイン）はいつでも変更できるので、直すときは削除ではなく編集で。

**なぜ2つのチャネルが同じプロバイダーでないといけないか**
